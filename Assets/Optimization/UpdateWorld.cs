...
public class UpdateWorld : MonoBehaviour
{
    ...
    Color[] colorArr = { Color.white, Color.black, Color.yellow, Color.green, Color.magenta, Color.blue };
        ...
        for (int y = 0; y < texture.height; y++)
        {
            for (int x = 0; x < texture.width; x++)
            {
                if (Mathf.PerlinNoise(x / 20.0f, y / 20.0f) * 100 < 40)
                    texture.SetPixel(x, y, colorArr[2]);
            }
        }
    ...
    void SimulateWorld(Texture2D texture)
    {
        for (int y = 0; y < texture.height; y++)
        {
            for (int x = 0; x < texture.width; x++)
            {
                var cacheCountNeighbourColor = CountNeighbourColor(x, y, colorArr[1], texture);

                //if a cell has more than 4 black neighbours make it blue
                //Commercial Property
                if (cacheCountNeighbourColor > 4)
                {
                    texture.SetPixel(x, y, colorArr[5]);
                }
                //if a cell has a black neighbour and is not black itself
                //set to green
                //Residential Property
                else if (cacheCountNeighbourColor > 0 && texture.GetPixel(x, y) == colorArr[0])
                {
                    texture.SetPixel(x, y, colorArr[3]);
                }
                //if near a black cell but the cell is already yellow
                //Mining Property
                if (cacheCountNeighbourColor > 0 && texture.GetPixel(x, y) == colorArr[2])
                {
                    texture.SetPixel(x, y, colorArr[4]);
                }
                ...
                if (cacheCountNeighbourColor == 0 &&
                    (texture.GetPixel(x, y) == colorArr[3] ||
                    texture.GetPixel(x, y) == Color.blue ||
                    texture.GetPixel(x, y) == colorArr[4]))
                {
                    texture.SetPixel(x, y, colorArr[0]);
                }
            }
        }
        texture.Apply();
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out ray))
            {
                ...
                DrawPixelLine((int)(ray.textureCoord.x * texture.width),
                                   (int)(ray.textureCoord.y * texture.height),
                                   (int)lastPoint.x, (int)lastPoint.y, colorArr[1], texture);
                texture.Apply();
                lastPoint = new Vector2((int)(ray.textureCoord.x * texture.width),
                                   (int)(ray.textureCoord.y * texture.height));
            }
        ...
        if (Input.GetMouseButton(1))
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out ray))
            {
                ...
                texture.SetPixel((int)(ray.textureCoord.x * texture.width),
                                   (int)(ray.textureCoord.y * texture.height), colorArr[0]);
                texture.Apply();
            }
        }
